...
const { finalprint } = require('./pcbFinalPrint02')
const { MongoClient, MongoClients } = require('mongodb');
request = require('request');
fs = require('fs');
// const url = 'mongodb://0.0.0.0:27017';
const url = 'mongodb+srv://smbarge:csswakad%408782@cluster-m01.px6yrsb.mongodb.net/kcb-02-2022?retryWrites=true&w=majority';
...
MongoClient.connect(url).then((client) => {
	const connect = client.db('kcb-02-2022');
//   console.log('connection is:', JSON.stringify(connect))
	// Connect to collection
	
	const collection = connect
		.collection('applications').find({"personalInfo.email":/aniketkate95@gmail.com/})
		.toArray().then((ans) => {
