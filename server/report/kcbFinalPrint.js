...
exports.finalprint = ({ application }) => {
    let aPromise = new Promise((resolve, reject) => {
        ...
        const getEssentialQualificationEnglish = () => {
            let title = {
                bold: true,
                fontSize: 14,
                text: 'Essential Qualification: Eng-TW-40/Hind-TW-30',
                alignment: "left",
                margin: [0, 30, 0, 10]
            }
            let body = [];
            ...
            qa[0].text = 'Sr.No.', qa[0].alignment = 'center', qa[0].bold = true,
                qa[1].text = 'Qualification', qa[1].alignment = 'center', qa[1].bold = true,
                qa[2].text = 'Typing Certificate', qa[2].alignment = 'center', qa[2].bold = true,
                qa[3].text = 'Certifying Authority', qa[3].alignment = 'center', qa[3].bold = true,
                qa[4].text = 'Month-Year(Exam)', qa[4].alignment = 'center', qa[4].bold = true,
                qa[5].text = 'Grade/Marks', qa[5].alignment = 'center', qa[5].bold = true,
                totalRows.table.body.push(qa)

            let qualification = application.essentialQualifications.filter((ele) => { return ele.qualification == "English Typing 40" || ele.qualification == "Hindi Typing 30" })
            console.log("qualification is........./", qualification)
            for (let i in qualification) {
                const q1 = JSON.parse(JSON.stringify(sixentry));
                q1[0].text = Number(i) + Number(1), q1[0].alignment = 'center',
                    q1[1].text = `${qualification[i].qualification}`, q1[1].alignment = 'center',
                    q1[2].text = qualification[i].certificateType, q1[2].alignment = 'center',
                    q1[3].text = `${qualification[i].certifyingAuthority == "Other" ? `Other-${qualification[i].certifyingAuthorityOther}` : qualification[i].certifyingAuthority}`, q1[3].alignment = 'center',
                    q1[4].text = `${qualification[i].examMonth}-${qualification[i].examYear}`, q1[4].alignment = 'center',
                    q1[5].text = qualification[i].gradeOrMarks, q1[5].alignment = 'center',
                    totalRows.table.body.push(q1)
            }
