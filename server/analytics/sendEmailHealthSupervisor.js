...
const doit = async () => {
  let data = fs.readFileSync("./healthSupervisorEmails.json", "utf8");
  let emailList = JSON.parse(data);
  let subject =
    "Pune Cantonment Board (Health Supervisor), Reminder to complete application before 4 April 2023";
  // console.log("emailList is :", emailList);
  emailList = [...emailList, "smbarge@gmail.com"];
  // emailList = ["smbarge@gmail.com"];
  let promises = emailList.map(async (e) => {
    ...
    const text = `Dear Candidate,
        <br>
        <br>
    You have registered for post of ‘Health Supervisor, Pune Cantonment Board’ on www.pcb01.parikshaworld.com with your email id ${e}. However, you have not completed your application form as of ’15 March 2023 11:40 AM’ and have not paid the fees. 
        <br>
        <br>

    If you believe you are qualified for this post as per advertisement and are interested in applying for the post, you must complete the application form including fee payment before 04 April 2023.
    Ignore this message, if you have completed the application form and have already paid the fees.
        <br>
        <br>
    There was some technical problem reported with post, however, the problem is now resolved. If you face any technical problems pertaining to application form, do not hesitate to contact support team.
        <br>
        <br>
    Pune Cantonment Board Support Team
    `;
  ...
  fs.writeFileSync(
    "./successReminderHealthSupervisor.json",
    JSON.stringify(successfulReminder, null, 2)
  );
  fs.writeFileSync(
    "./failedReminderHealthSupervisor.json",
