<script>
  ...
  import { application, token, availablePosts } from "../../../stores/stores";
  ...
  console.log("available posts:", JSON.stringify($availablePosts));
  let postName = "";
  ...
  onMount(async () => {
    ...
    postName = $availablePosts
      ? $availablePosts.find((ap) => ap.postId == postId).postName
      : "Post Name";
  ...
  {#if dataloaded}
    <div class="bg-orange-400 flex text-center">
      <div class="grid md:grid-cols-10">
        ...
        >
          Application for post of {postName} - Pune Cantonment Board
        </div>
      ...
      <div class="mr-4">
        <button
          type="submit"
          class=" mt-8 w-20 p-2 h-18 text-center flex justify-center bg-white hover:bg-orange-400 shadow-lg text-orange-500 hover:text-orange-50 text-sm font-semibold rounded "
          on:click={() => {
            $token = "";
            goto("/signin");
          }}
