<script>
  ...
  const validateInput = () => {
    ...
    if (inputQualification.selected == "8th") {
      if (!validateTextField({ field: inputSchoolName }))
        (inputSchoolName = { ...inputSchoolName }), (valid = false);
    }
  ...
  function onAdd() {
    ...
    let essentialQualification = {
      ...
      schoolName:
        inputQualification.selected == "8th" ? inputSchoolName.value : "",
