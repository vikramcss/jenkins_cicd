<script>
  ...
  const qualificationOptions = {
    Qualifications: ["10th", "Driving Licence LMV", "Driving Licence HMV"],
  };
  ...
  let inputSchoolName = {
    ...
    value: essentialQualification.schoolName,
    validate: true,
    readonly: false,
    error: null,
    placeholderValue: "Name Of School",
    error_mt: false,
    paddingsize: 0,
    validator: {
      method: (value) => {
        inputSchoolName.error = null;
        if (value.length < 2) {
          inputSchoolName.error = "Name of School can not be empty";
          return false;
        }
        return true;
      },
      errormessage: "Name of School can not be empty",
    },
  };
  let inputBoardName = {
    label: "Name of Board",
  ...
  let inputDLNumber = {
    ...
    value: essentialQualification.dLNumber,
    validate: true,
    readonly: false,
    error: null,
    placeholderValue: "Driving Licence Number",
    error_mt: false,
    paddingsize: 0,
    validator: {
      method: (value) => {
        inputDLNumber.error = null;
        if (value.length < 2) {
          inputDLNumber.error = "Driving Licence can not be empty";
          return false;
        }
        return true;
      },
      errormessage: "Driving Licence can not be empty",
    },
  };
  let inputValidTill = {
    label: "Valid Till",
    value: essentialQualification.validTill,
    validate: true,
    readonly: false,
    error: null,
    placeholderValue: "Valid Till",
    error_mt: false,
    paddingsize: 0,
    validator: {
      method: (value) => {
        inputValidTill.error = null;
        if (value.length < 2) {
          inputValidTill.error = "Valid Till can not be empty";
          return false;
        }
        return true;
      },
      errormessage: "Valid Till can not be empty",
    },
  };
  ...
  const validateInput = () => {
    ...
    if (inputQualification.selected == "10th") {
    ...
    if (
      inputQualification.selected == "Driving Licence LMV" ||
      inputQualification.selected == "Driving Licence HMV"
    ) {
      if (inputDLNumber.value.length < 4) {
        inputDLNumber.error = "Enter Proper DL Number";
        alertMessage = "Complete all Fields";
        valid = false;
      }
      if (inputValidTill.value == null || inputValidTill.value == "") {
        inputValidTill.error = "Date is mandatory";
        valid = false;
      }
      if (
        DateTime.fromObject({
          year: inputValidTill.value.split("-")[2],
          month: inputValidTill.value.split("-")[1],
          day: inputValidTill.value.split("-")[0],
        }) < DateTime.fromObject({ year: 2023, month: 3, day: 4 })
      ) {
        alertMessage = "Your Driving Licence is Expired";
        inputValidTill.error = "Your Driving Licence is Expired";
        valid = false;
      }
    }
    return valid;
  ...
  function onEdit() {
    ...
    if (inputQualification.selected == "10th") {
      let _essentialQualification = {
        qualification: inputQualification.selected,
        yearOfPassing: inputYearOfPassing.value,
        boardName:
          inputQualification.selected == "10th" ? inputBoardName.value : "",
        marksType: inputMarksType.value,
      };

      if (inputMarksType.value === "Percentage") {
        _essentialQualification.marks = inputMarks.value;
        _essentialQualification.percentage = inputPercentage.value;
      } else {
        _essentialQualification.cgpa = inputCGPA.value;
      }
      essentialQualification = _essentialQualification;
    } else {
      let _essentialQualification = {
        qualification: inputQualification.selected,
        dLNumber: inputDLNumber.value,
        validTill: inputValidTill.value,
      };
      ...
      >
        ...
        <div class="relative p-6">
          <div class="grid grid-cols-1 md:grid-cols-2  md:gap-3  ">
            ...
            />
            {#if inputQualification.selected}
              {#if inputQualification.selected == "7th"}
                <TextInput
                  bind:label={inputSchoolName.label}
                  bind:value={inputSchoolName.value}
                  validate={inputSchoolName.validate}
                  readonly={inputSchoolName.readonly}
                  bind:error={inputSchoolName.error}
                  placeholderValue={inputSchoolName.placeholderValue}
                  error_mt={inputSchoolName.error_mt}
                  bind:paddingsize={inputSchoolName.paddingsize}
                  validator={inputSchoolName.validator}
                />
              {:else if inputQualification.selected == "10th"}
                <TextInput
              ...
              {:else}
                <TextInput
                  bind:label={inputDLNumber.label}
                  bind:value={inputDLNumber.value}
                  validate={inputDLNumber.validate}
                  bind:error={inputDLNumber.error}
                  placeholderValue={inputDLNumber.placeholderValue}
                  error_mt={inputDLNumber.error_mt}
                  bind:paddingsize={inputDLNumber.paddingsize}
                  validator={inputDLNumber.validator}
                />

                <DateInput
                  label={inputValidTill.label}
                  bind:value={inputValidTill.value}
                  bind:error={inputValidTill.error}
                  validator={inputValidTill.validator}
                />
              {/if}
            {/if}
          </div>
