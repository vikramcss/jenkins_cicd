<script>
  ...
  const login = async () => {
    ...
    if (!username || username === "")
      return (alert = "Registered Email id  cannot be null");
    if (!password || password === "")
      return (alert = "password cannot be null");
    if (!captcha.text || captcha.text === "")
      return (alert = "captcha cannot be null");

    const { error, errorMsg, token, success } = await api.login({
      login: username.toLowerCase(),
      password,
      captcha: { id: captcha.id, text: captcha.text },
    });
    console.log("login returned:", error, " token: ", token);
...
<main>
  <div class="grid grid-col-12 h-auto min-h-screen bg-orange-200">
    <div class="container mx-auto p-5">
      <div
        class="max-w-sm mx-auto bg-white px-5 pt-2 pb-3 rounded-lg shadow-xl "
      >
        <div>
          <img class="object-contain h-28 w-full" src="/PCBlogo.png" alt="pcb-logo" />
          <h1 class="text-center font-bold text-lg">
            Pune Cantonment Board - 34 Posts
             <span class="text-xs"
              > (Adv: Dated(Employment News): 04<sup>th</sup> March 2023  )
            </span>
          </h1>
          <form autocomplete="off" on:submit|preventDefault={login}>
            <div
              class="mt-5 outline relative border-2 white focus-within:border-orange-500 rounded"
            >
              ...
              <input
                ...
                class="block p-2 h-full w-full mb-0 text-lg appearance-none border-none focus:outline-none border-1 border-orange-100 bg-transparant"
              />
            ...
            <div
              class="flex justify-end mt-4 uppercase font-semibold text-xs w-full "
            >
              <label for="viewpassword" class="mt-1">view</label>
              <input
                id="viewpassword"
                tabindex="-1"
                type="checkbox"
                class="my-1 ml-1 form-checkbox text-orange-500"
                bind:checked={viewpassword}
              />
            </div>
            <div
              class=" outline relative border-2 white focus-within:border-orange-500 rounded"
            >
              {#if viewpassword}
                <input
                  ...
                  placeholder=""
                  autocomplete="off"
                  class="block p-2 h-full w-full  mb-0 text-lg appearance-none border-none focus-within:border-orange-500 bg-transparant"
                />
              {:else}
                <input
                  bind:this={passwordEl}
            ...
            <div class="mt-1">
              <h1 class="text-right text-sm">
                <a
                  tabindex="-1"
                  class="text-orange-400 underline hover:text-orange-600"
                  href="/forgotpassword"
                >
                  Forgot Password?
                </a>
            ...
            >
              <div class="flex flex-row">
                ...
                <!-- class="flex border-none focus:outline-red-none bg-none rounded-full mt-2 mr-2 " -->
              ...
              <div class="flex flex-row">
                <div
                  class="ml-2 mt-3 mb-2 outline relative border-2 white focus-within:border-orange-500 rounded"
                >
                  <input
                    bind:this={captchaEl}
                    type="text"
            ...
            <button
              type="submit"
              class=" mt-2 w-full text-center bg-orange-500 hover:bg-orange-400  shadow-lg text-white font-bold py-2 px-4 rounded "
            >
              LOGIN
            </button>
            <button
              ...
              type="button"
              class=" mt-2 w-full text-center bg-white  shadow-lg text-orange-500 hover:text-gray-700 font-bold py-2 px-4 rounded  border-2 border-orange-600 hover:border-orange-400 "
            >
